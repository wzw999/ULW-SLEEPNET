...
def main():
    ...
    logger.info(f"Looking for EDF files in directory: {args.data_dir}")
    psg_fnames = glob.glob(os.path.join(args.data_dir, "*PSG.edf"))
    ann_fnames = glob.glob(os.path.join(args.data_dir, "*Hypnogram.edf"))
    logger.info(f"Found {len(psg_fnames)} PSG files")
    logger.info(f"Found {len(ann_fnames)} Hypnogram files")
    
    if len(psg_fnames) == 0:
        logger.error(f"No PSG files found in {args.data_dir}")
        logger.info("Please check if the data directory path is correct and contains *PSG.edf files")
        return
    
    if len(ann_fnames) == 0:
        logger.error(f"No Hypnogram files found in {args.data_dir}")
        logger.info("Please check if the data directory path is correct and contains *Hypnogram.edf files")
        return
